...
<div @include('crud::inc.field_wrapper_attributes') >
    ...
    <div class="div">
        <div class="row upload-image">
            <div class="col-md-6">
                <img src="{{ old($field['name']) ? old($field['name']) : (isset($field['value']) ? url($field['value']) : (isset($field['default']) ? url($field['default']) : '' )) }}"  id="{{ $field['name'] }}-upload-image">
            </div>
        </div>
    </div>
    ...
    <div class="well well-sm">
    	<a target="_blank" href="{{ url($field['value']) }}">{{ $field['value'] }}</a>
    ...
    @push('crud_fields_scripts')
        ...
        <script>
	        $("#{{ $field['name'] }}_file_clear_button").click(function(e) {
	        	...
	        	$('#{{ $field['name'] }}-upload-image').attr('src','');
	        ...
	        $("#{{ $field['name'] }}_file_input").change(function() {
	        	...
	        	if(this.files && this.files[0]){
	        	    ...
	        	    reader.onload = function(e){
                        $('#{{ $field['name'] }}-upload-image').attr('src',e.target.result);
                    }
